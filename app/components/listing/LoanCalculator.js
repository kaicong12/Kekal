...
import { Col, InputNumber, Row, Slider, Select, message } from "antd";
...
const SelectLoanPeriod = ({ title, handleChangeSelect }) => (
  <>
    <div className="mb20" style={{ fontWeight: "600" }}>
      {title}
    </div>
    <Select
      defaultValue="2"
...
const LoanCalculator = ({ motorcycle }) => {
  ...
  const getMonthlyPayment = useCallback(
    (downPaymentPercent, annualInterestRate, loanTermYears) => {
  ...
  return (
    <>
      ...
      <p className="first-para mb30">
        Find out if you can afford your dream car using our user friendly car
        loan calculator.
      </p>
      <IntegerStep
        title="Downpayment (%)"
      ...
      <SelectLoanPeriod
        ...
        handleChangeSelect={setLoanYear}
      />

      <div className={styles.displayContainer}>
        <div className={styles.displayDiv}>
          <strong style={{ display: "block", fontSize: "20px" }}>
            RM {(priceFloat * downPayment) / 100}
          </strong>
        ...
        <div className={styles.displayDiv}>
          <strong
            style={{ display: "block", fontSize: "20px", marginTop: "20px" }}
          >
            RM{Math.round(monthlyPayment * 100) / 100}
          </strong>
        ...
        <button
          type="button"
          className={`btn btn-block btn-thm mt30 mb20 ${styles.loanEnquiryButton}`}
          onClick={() => {
...
Loan Details:
- Down Payment: ${downPayment}% (RM ${Math.round(
              (priceFloat * downPayment) / 100
            )})
- Loan Period: ${loanYear} years
- Estimated Monthly Payment: RM ${Math.round(monthlyPayment * 100) / 100}
